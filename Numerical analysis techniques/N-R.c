...
main()
{
    ...
    do {
        i += 1;
        double detA = det(0, 3, x4, x3, 1, 2, 1, 4, 3 * pow(x1, 2), 0, 3 * pow(x3, 2), 1, 2 * (x1 + x2), 2 * x1, 0, 3 * pow(x4, 2));
        dx1 = det(-f1(x1, x2, x3, x4), 3, x4, x3, -f2(x1, x2, x3, x4), 2, 1, 4, -f3(x1, x2, x3, x4), 0, 3 * pow(x3, 2), 1, -f4(x1, x2, x3, x4), 2 * x1, 0, 3 * pow(x4, 2)) / detA;
        dx2 = det(0, -f1(x1, x2, x3, x4), x4, x3, 1, -f2(x1, x2, x3, x4), 1, 4, 3 * pow(x1, 2), -f3(x1, x2, x3, x4), 3 * pow(x3, 2), 1, 2 * (x1 + x2), -f4(x1, x2, x3, x4), 0, 3 * pow(x4, 2)) / detA;
        dx3 = det(0, 3, -f1(x1, x2, x3, x4), x3, 1, 2, -f2(x1, x2, x3, x4), 4, 3 * pow(x1, 2), 0, -f3(x1, x2, x3, x4), 1, 2 * (x1 + x2), 2 * x1, -f4(x1, x2, x3, x4), 3 * pow(x4, 2)) / detA;
        dx4 = det(0, 3, x4, -f1(x1, x2, x3, x4), 1, 2, 1, -f2(x1, x2, x3, x4), 3 * pow(x1, 2), 0, 3 * pow(x3, 2), -f3(x1, x2, x3, x4), 2 * (x1 + x2), 2 * x1, 0, -f4(x1, x2, x3, x4)) / detA;
        x1 += dx1;
        x2 += dx2;
        x3 += dx3;
        x4 += dx4;
